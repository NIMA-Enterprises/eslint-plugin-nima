...
import { generateBooleanSuggestion } from "@utility/naming-helpers";
...
export const create = (
    ...
    const checkNameOnce = ({
        ...
        if (!hasValidBooleanPrefix(name)) {
            ...
            context.report({
                data: {
                    name,
                    suggestion: generateBooleanSuggestion(name),
                },
    ...
    const isReturnBooleanType = (node: TSESTree.FunctionLike) => {
        try {
    ...
    const checkFunctionLikeExpression = (
    ...
    const checkFunctionDeclaration = (node: TSESTree.FunctionDeclaration) => {
        ...
        if (node.id && isReturnBooleanType(node)) {
    ...
    const checkFunctionExpression = (node: TSESTree.FunctionExpression) => {
        checkFunctionLikeExpression(node);
    };
            ...
            if (
                ...
                (isBooleanType(prop.value) ||
                    ((prop.value.type === AST_NODE_TYPES.FunctionExpression ||
                        ...
                        isReturnBooleanType(prop.value)))
            ) {
    ...
    const checkVariableDeclarator = (node: TSESTree.VariableDeclarator) => {
    ...
    return {
        ArrowFunctionExpression: checkFunctionLikeExpression,
